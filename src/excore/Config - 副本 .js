    ...
    Config.prototype = /**@lends Config#*/ {
        ...
        */
        set: function (name, config) {
            ...
            var name$defaulted = meta.name$defaulted;
            ...
            function setItem(name, config) {
                ...
                //首次设置，先使用 KISP 内部的默认配置。
                //这样检测，是针对业务层在调用 get 之前就先 set。
                var defaulted = name$defaulted[name];
                if (!defaulted) {
                    name$config[name] = require('Config.' + name);
                    name$defaulted[name] = true;
                }
